...
function normalizeNode(node: NodeWithPeople | null): NodeWithPeople {
  if (!node) {
    throw new Error("No se encontró la caja después de guardar los cambios.");
  }
  return node;
}
...
async function assertChartBelongsToSchool(orgChartId: string, schoolSlug: string) {
  ...
  const chart = await prisma.orgChart.findFirst({
...
export async function createPucaraStarterChartAction(input: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const chart = await tx.orgChart.create({
      data: {
        schoolId: school.id,
        title,
        year,
    ...
    const makeNode = (data: Omit<Prisma.OrgNodeUncheckedCreateInput, "orgChartId">) => tx.orgNode.create({
...
export async function savePucaraPositionsAction(input: {
  ...
  const touched = await prisma.orgNode.findMany({
    where: { id: { in: input.positions.map((position) => position.nodeId) } },
    include: { orgChart: { select: { schoolId: true } } },
  });
  if (touched.length !== input.positions.length || touched.some((node) => node.orgChart?.schoolId !== schoolId)) {
...
export async function updatePucaraNodeAction(input: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (requestedPersonId && requestedPersonId !== "__new__") {
      const selected = await tx.person.findFirst({
        where: {
    ...
    const node = await tx.orgNode.update({
      where: { id: input.nodeId },
      data: {
...
export async function createPucaraChildAction(input: {
  ...
  const nextOrder =
    (await prisma.orgNode.count({ where: { orgChartId: input.orgChartId } })) + 1;
  const siblingCount = await prisma.orgEdge.count({
...
export async function createPucaraRelationAction(input: {
  ...
  const existing = await prisma.orgEdge.findFirst({
    where: {
      orgChartId: input.orgChartId,
      type: input.type,
      OR: [
...
export async function updatePucaraRelationAction(input: {
  schoolSlug: string;
  edgeId: string;
  type: "DECISION" | "COLABORACION";
}) {
  const edge = await prisma.orgEdge.findUnique({ where: { id: input.edgeId } });
  if (!edge) throw new Error("No se encontró el vínculo seleccionado.");
  await assertNodeBelongsToSchool(edge.sourceId, input.schoolSlug);
  if (edge.type === "JERARQUICA") {
    throw new Error("La dependencia jerárquica se cambia desde ‘Depende de’.");
  }

  const duplicate = await prisma.orgEdge.findFirst({
    where: {
      orgChartId: edge.orgChartId,
      id: { not: edge.id },
      type: input.type,
      OR: [
  ...
  if (duplicate) {
    throw new Error(
      input.type === "DECISION"
        ? "Estas cajas ya tienen un vínculo Integra."
        : "Estas cajas ya tienen un vínculo Colabora.",
    );
  }

  const updated = await prisma.orgEdge.update({
    where: { id: edge.id },
    data: {
      type: input.type,
      label: input.type === "DECISION" ? "Integra" : "Colabora",
...
export async function normalizePucaraHierarchyAction(input: {
  ...
  await assertChartBelongsToSchool(input.orgChartId, input.schoolSlug);
  let nodes = await prisma.orgNode.findMany({
    where: { orgChartId: input.orgChartId },
    select: { id: true, title: true, area: true, positionX: true, positionY: true },
  });
  if (nodes.length <= 1) {
    return { created: 0, rebuilt: false, hierarchyEdges: [] as Array<{ id: string; sourceId: string; targetId: string; type: "JERARQUICA"; label: string | null }> };
  }
  ...
  if (isBuenAyreSchoolSlug(input.schoolSlug)) {
    ...
    let nextOrder = await prisma.orgNode.count({ where: { orgChartId: input.orgChartId } });
    const createdTeamNodes: Array<ReturnType<typeof normalizeNode>> = [];

    for (const definition of teamDefinitions) {
      const direction = nodes.find((node) => {
        const title = normalizeBuenAyreTitle(node.title);
        return definition.directionPatterns.some((pattern) => pattern.test(title));
      });
      const existingTeam = nodes.find((node) => {
        const title = normalizeBuenAyreTitle(node.title);
    ...
    const hierarchyEdges = await prisma.orgEdge.findMany({
      where: { orgChartId: input.orgChartId, type: "JERARQUICA" },
  ...
  const hierarchy = await prisma.orgEdge.findMany({
    where: { orgChartId: input.orgChartId, type: "JERARQUICA" },
    select: { sourceId: true, targetId: true },
  });
  const incoming = new Set(hierarchy.map((edge) => edge.targetId));
  const children = new Map<string, string[]>();
  ...
  const council = nodes.find((node) => norm(node.title).includes("consejo de direccion"));
  const director = nodes.find((node) => norm(node.title).includes("direccion general") || norm(node.title).includes("director general"));
  const root = council ?? director ?? [...nodes].sort((a, b) => Number(a.positionY) - Number(b.positionY))[0];
  ...
  const ordered = [...nodes].sort((a, b) => Number(a.positionY) - Number(b.positionY));
  ...
  if (additions.length) {
    await prisma.orgEdge.createMany({ data: additions, skipDuplicates: true });
  }
...
export async function deletePucaraNodeAction(input: {
  ...
  await assertNodeBelongsToSchool(input.nodeId, input.schoolSlug);
  const childCount = await prisma.orgEdge.count({
...
export async function upsertPucaraMemberAction(input: {
  ...
  await prisma.$transaction(async (tx) => {
    if (input.memberId) {
      const membership = await tx.orgNodeMember.findFirst({
    ...
    const existing = await tx.orgNodeMember.findFirst({
...
function areaForNode(node: PucaraHitoNode): NodeAreaValue {
...
export async function importPucaraHitoAction(input: { schoolSlug: string }) {
  ...
  const nodeRows: Prisma.OrgNodeCreateManyInput[] = PUCARA_HITO_NODES.map((template, index) => {
  ...
  await prisma.$transaction(
    async (tx) => {
      await tx.orgEdge.deleteMany({ where: { orgChartId: chart.id } });
      await tx.orgNode.deleteMany({ where: { orgChartId: chart.id } });

      if (nodeRows.length) {
        await tx.orgNode.createMany({ data: nodeRows });
      }
      if (memberRows.length) {
        await tx.orgNodeMember.createMany({ data: memberRows });
      }
      if (edgeRows.length) {
        await tx.orgEdge.createMany({ data: edgeRows });
      }
    },
    { maxWait: 10_000, timeout: 30_000 },
  );
