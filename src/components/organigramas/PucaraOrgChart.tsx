...
function ChartInner(props: Props) {
  ...
  const [showEditorGuide, setShowEditorGuide] = useState(props.initialMode === "edit");
  ...
  const changeMode = (nextMode: Mode) => {
    ...
    if (nextMode === "edit") {
      if (mode !== "edit") setShowEditorGuide(true);
  ...
  const openQuickPeople = () => {
    if (!currentNode) {
      setMessage({ type: "error", text: "Primero tocá una caja del organigrama para elegir dónde agregar la persona." });
      return;
    }
    setEditorSection("team");
    setEditingMemberId("new");
  };

  const openQuickRelations = () => {
    if (!currentNode) {
      setMessage({ type: "error", text: "Primero tocá una caja para elegir desde qué función querés crear el vínculo." });
      return;
    }
    setEditorSection("relations");
  };
  ...
  const removeRelation = (edgeId: string) => {
    if (!window.confirm("¿Querés quitar este vínculo? La caja y las personas no se eliminan.")) return;
    runAction(async () => {
  ...
  const currentHasPeople = Boolean(currentNode?.person || currentNode?.members.length);
  const currentHasParent = Boolean(currentNode && (currentIsRoot || currentParentId));
  const currentHasRelations = currentRelations.length > 0;
  ...
  return (
    <main className="min-h-screen bg-[#eef2f7] p-2 md:p-5">
              ...
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => openCreateNode(selectedId || roots[0] || null)} disabled={!props.orgChartId || sourceNodes.length === 0} className="inline-flex items-center gap-2 rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-40">
                    <Plus className="h-4 w-4" /> Agregar función
                  </button>
                  <button type="button" onClick={openQuickPeople} className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-black text-amber-800 transition hover:bg-amber-100">
                    <UserPlus className="h-4 w-4" /> Agregar persona
                  </button>
                  <button type="button" onClick={openQuickRelations} className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-black text-blue-800 transition hover:bg-blue-100">
                    <Link2 className="h-4 w-4" /> Agregar vínculo
                  </button>
                  <button type="button" onClick={() => setShowEditorGuide(true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 transition hover:border-blue-300 hover:text-blue-700">
                    <Info className="h-4 w-4" /> Guía rápida
                  </button>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => setSimpleEditor((current) => !current)} className={`rounded-xl border px-3.5 py-2.5 text-xs font-black transition ${simpleEditor ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-white text-slate-600"}`}>
                    {simpleEditor ? "Modo simple" : "Modo completo"}
                  </button>
                  <details className="relative">
                    <summary className="cursor-pointer list-none rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-600 transition hover:bg-slate-50">Más opciones</summary>
                    <div className="absolute right-0 top-full z-50 mt-2 w-[300px] space-y-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
                      <button type="button" onClick={autoOrderEditor} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 transition hover:border-blue-300 hover:text-blue-700"><LayoutGrid className="h-4 w-4" /> Ordenar automáticamente</button>
                      <button type="button" onClick={savePositions} disabled={isPending} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white transition hover:bg-emerald-700 disabled:opacity-60">{isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Guardar posiciones</button>
                      <button type="button" onClick={() => setShowQualityPanel((current) => !current)} className={`inline-flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-black transition ${qualityReady ? "border-emerald-200 bg-emerald-50 text-emerald-800 hover:bg-emerald-100" : "border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100"}`}><Check className="h-4 w-4" /> Revisar antes de terminar</button>
                      {isBuenAyre || missingDependencyCount > 0 ? (
                        <button type="button" onClick={completeMissingDependencies} disabled={isPending} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-4 py-2.5 text-sm font-black text-amber-800 transition hover:bg-amber-100 disabled:opacity-60"><Link2 className="h-4 w-4" /> {isBuenAyre ? "Reacomodar Buen Ayre" : `Completar dependencias (${missingDependencyCount})`}</button>
                      ) : null}
                      {props.schoolSlug === "pucara" ? (
                        <button type="button" onClick={importHito} disabled={isPending} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#1C3A62] px-4 py-2.5 text-sm font-black text-white transition hover:bg-[#152f50] disabled:opacity-60"><UsersRound className="h-4 w-4" /> Importar estructura Pucará</button>
                      ) : null}
                    </div>
                  </details>
                </div>
              </div>
              <p className="mt-2 text-xs font-semibold text-slate-500">Tocá una caja para editarla. Los textos se guardan con el botón azul; cuando movés una caja, su posición se guarda automáticamente.</p>
            </div>
            {showQualityPanel ? (
                ...
                <>
                  <div className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 p-5 backdrop-blur">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-600">Estás editando</p>
                        <h2 className="mt-1 text-xl font-black text-slate-950">{currentNode.title}</h2>
                        <p className="mt-1 text-sm font-semibold text-slate-500">Hacelo en tres pasos. Podés volver a cualquiera cuando quieras.</p>
                      </div>
                    ...
                    <div className="mt-4 grid grid-cols-3 rounded-xl bg-slate-100 p-1">
                      <button type="button" onClick={() => setEditorSection("function")} className={`rounded-lg px-2 py-2 text-sm font-black transition ${editorSection === "function" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>1. Datos {currentHasParent ? "✓" : ""}</button>
                      <button type="button" onClick={() => setEditorSection("team")} className={`rounded-lg px-2 py-2 text-sm font-black transition ${editorSection === "team" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>2. Personas {currentHasPeople ? "✓" : ""}</button>
                      <button type="button" onClick={() => setEditorSection("relations")} className={`rounded-lg px-2 py-2 text-sm font-black transition ${editorSection === "relations" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>3. Vínculos {currentHasRelations ? "✓" : ""}</button>
                    </div>
                  </div>
                  ...
                  {editorSection === "function" ? (
                    <form action={saveNode} key={`node-${currentNode.id}`} className="space-y-4 p-5">
                      {simpleEditor ? (
                        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm font-semibold leading-relaxed text-blue-900"><strong>Lo importante:</strong> nombre de la función, de quién depende y quién es la persona responsable. El resto se puede completar después.</div>
                      ) : null}
                      ...
                      <label className="block">
                        <span className="mb-2 block text-[11px] font-black uppercase tracking-[0.14em] text-slate-500">¿De quién depende esta función?</span>
                        <select
                          name="parentNodeId"
                        ...
                        <span className="mt-1.5 block text-xs font-semibold text-slate-400">Elegí la función que está inmediatamente arriba en el organigrama. La página acomoda la conexión sola.</span>
                      </label>

                      <Field label="Nombre de la función" name="title" defaultValue={currentNode.title} />
                      ...
                      <details open={!simpleEditor} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <summary className="cursor-pointer list-none text-sm font-black text-slate-700">Más información <span className="font-semibold text-slate-400">(opcional)</span></summary>
                        <div className="mt-4 space-y-4">
                          ...
                          <Field label="Correo" name="email" type="email" defaultValue={currentNode.person?.email || ""} placeholder="nombre@colegio.edu.ar" icon={<Mail className="h-4 w-4" />} />
                          <Field label="Foto" name="photoUrl" defaultValue={currentNode.person?.photoUrl || ""} placeholder="juan-perez.jpg" icon={<ImageIcon className="h-4 w-4" />} />
                          <PhotoHelp />
                        </div>
                      </details>
                      ...
                      <button type="submit" disabled={isPending} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 py-3 text-sm font-black text-white transition hover:bg-blue-800 disabled:opacity-60">
                        {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Guardar cambios
                      </button>

                      {simpleEditor ? (
                        <button type="button" onClick={() => setEditorSection("team")} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-black text-blue-700 transition hover:bg-blue-100">Seguir con las personas <ArrowRight className="h-4 w-4" /></button>
                      ) : null}
                    ...
                    <div className="space-y-4 p-5">
                      {simpleEditor ? (
                        <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm font-semibold leading-relaxed text-amber-900">Agregá solamente a las personas que realmente trabajan dentro de esta función. Si es una sola responsable, alcanza con la persona del paso anterior.</div>
                      ) : null}
                      ...
                      {simpleEditor ? (
                        <button type="button" onClick={() => setEditorSection("relations")} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-black text-blue-700 transition hover:bg-blue-100">Listo con personas · seguir a vínculos <ArrowRight className="h-4 w-4" /></button>
                      ) : null}
                    </div>
                  ) : (
                    <div className="space-y-5 p-5">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-black text-slate-950">¿Esta función trabaja especialmente con otra?</p>
                            <p className="mt-1 text-xs font-semibold leading-relaxed text-slate-500"><strong>Colabora</strong> significa que trabajan juntas. <strong>Integra</strong> significa que esta función forma parte de un equipo, comité o espacio de decisión. Estos vínculos no cambian quién depende de quién.</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <form action={addRelation} className="space-y-3 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
                        ...
                        <fieldset>
                          <legend className="mb-2 text-xs font-black uppercase tracking-[0.11em] text-slate-500">¿Qué tipo de vínculo es?</legend>
                          <div className="grid gap-2 sm:grid-cols-2">
                            <label className="cursor-pointer rounded-2xl border-2 border-blue-100 bg-white p-3 transition has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 has-[:checked]:ring-4 has-[:checked]:ring-blue-100"><input type="radio" name="relationType" value="COLABORACION" defaultChecked className="sr-only" /><span className="block text-sm font-black text-blue-800">Colabora</span><span className="mt-1 block text-xs font-semibold leading-relaxed text-slate-500">Trabajan juntas o se ayudan habitualmente.</span></label>
                            <label className="cursor-pointer rounded-2xl border-2 border-red-100 bg-white p-3 transition has-[:checked]:border-red-500 has-[:checked]:bg-red-50 has-[:checked]:ring-4 has-[:checked]:ring-red-100"><input type="radio" name="relationType" value="DECISION" className="sr-only" /><span className="block text-sm font-black text-red-800">Integra</span><span className="mt-1 block text-xs font-semibold leading-relaxed text-slate-500">Forma parte de un equipo, comité o espacio de decisión.</span></label>
                          </div>
                        </fieldset>
                        <label className="block">
      ...
      {showEditorGuide ? (
        <EditorGuideModal onClose={() => setShowEditorGuide(false)} onAddFunction={() => { setShowEditorGuide(false); openCreateNode(selectedId || roots[0] || null); }} />
      ) : null}
...
function EditorGuideModal({ onClose, onAddFunction }: { onClose: () => void; onAddFunction: () => void }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <div className="w-full max-w-[680px] overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5 md:p-6"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Guía rápida</p><h2 className="mt-1 text-2xl font-black text-slate-950">Editar el organigrama es hacer tres cosas</h2><p className="mt-2 text-sm font-semibold leading-relaxed text-slate-500">No necesitás saber de sistemas ni acomodar líneas manualmente.</p></div><button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-600 transition hover:bg-slate-200"><X className="h-5 w-5" /></button></div>
        <div className="grid gap-3 p-5 md:grid-cols-3 md:p-6"><GuideStep number="1" title="Elegí una caja">Tocá la función que querés cambiar. El panel derecho se abre solo.</GuideStep><GuideStep number="2" title="Completá los pasos">Primero Datos, después Personas y por último Vínculos. Los ✓ te muestran qué ya está completo.</GuideStep><GuideStep number="3" title="Guardá">Usá el botón azul cuando cambies textos o personas. Si movés una caja, la posición se guarda automáticamente.</GuideStep></div>
        <div className="border-t border-slate-200 bg-slate-50 p-5 md:flex md:items-center md:justify-between md:gap-4"><p className="text-xs font-semibold leading-relaxed text-slate-500">Para funciones poco frecuentes usá <strong>Más opciones</strong>. No hace falta entrar ahí para la edición normal.</p><div className="mt-3 flex shrink-0 flex-col gap-2 sm:flex-row md:mt-0"><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-100">Entendido</button><button type="button" onClick={onAddFunction} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 py-3 text-sm font-black text-white transition hover:bg-blue-800"><Plus className="h-4 w-4" /> Agregar una función</button></div></div>
      </div>
    </div>
  );
}
...
function GuideStep({ number, title, children }: { number: string; title: string; children: ReactNode }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-700 text-sm font-black text-white">{number}</div><p className="mt-3 text-sm font-black text-slate-950">{title}</p><p className="mt-1 text-xs font-semibold leading-relaxed text-slate-500">{children}</p></div>;
}
      ...
      <div className="w-full max-w-[720px] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5 md:p-6">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black text-slate-950">{step === 1 ? "¿Debajo de qué función va?" : "¿Qué querés agregar?"}</h2>
            <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-500">
              {step === 1
                ? "Elegí la función que va inmediatamente arriba. La página arma la conexión sola."
                : "Elegí la opción más parecida. Después podés cambiar el nombre y completar las personas sin empezar de nuevo."}
            </p>
          </div>
          <button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-600 transition hover:bg-slate-200"><X className="h-5 w-5" /></button>
        </div>
...
function PhotoHelp({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`rounded-xl bg-blue-50 text-xs font-semibold leading-relaxed text-blue-700 ${compact ? "p-2.5" : "p-3"}`}>
      <p>La foto es opcional. Si no la tenés ahora, dejá el campo vacío y podés completarla más adelante.</p>
      <details className="mt-2"><summary className="cursor-pointer font-black">Cómo cargar una foto</summary><p className="mt-2">Podés usar una URL https o guardar la imagen en <strong>public/images/personas</strong> y escribir, por ejemplo, <strong>juan-perez.jpg</strong>.</p></details>
    </div>
  );
}
