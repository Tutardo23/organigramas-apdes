...
export function LegacyOrgChartCanvas({ nodes, edges, schoolSlug }: Props) {
  ...
  const flowEdges = useMemo(() => edges.map(buildEdge), [edges]);
  const totalPeople = selectedNode
    ...
    : 0;
  const totalHours =
    selectedNode?.members?.reduce(
      (acc, member) => acc + (member.weeklyHours ?? 0),
      0,
    ) ||
    selectedNode?.weeklyHours ||
    null;
        ...
        {selectedNode ? (
          <div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <InfoPill
                ...
                label={areaLabels[selectedNode.area] ?? selectedNode.area}
              />
              <InfoPill
                icon={<UsersRound className="h-4 w-4" />}
                label={`${totalPeople} personas`}
              />
              {totalHours !== null ? (
                <InfoPill
            ...
            {selectedNode.person ? (
              <div className="mt-5 rounded-[1.4rem] border border-blue-100 bg-blue-50 p-4">
                ...
                <p className="mt-2 text-lg font-black text-blue-950">
                  {`${selectedNode.person.firstName} ${selectedNode.person.lastName}`.trim()}
                </p>
                {selectedNode.person.email ? (
                  <p className="mt-2 flex items-center gap-2 text-sm font-semibold text-blue-800">
                    <Mail className="h-4 w-4" />
                    {selectedNode.person.email}
                  </p>
                ) : null}
                {selectedNode.person.phone ? (
                  <p className="mt-2 flex items-center gap-2 text-sm font-semibold text-blue-800">
            ...
            <div className="mt-5 space-y-4">
              {selectedNode.formalRole ? (
                <DetailBlock
                  label="Cargo formal"
                  value={selectedNode.formalRole}
                />
              ) : null}
              {selectedNode.realFunction ? (
                <DetailBlock
                  label="Función real"
                  value={selectedNode.realFunction}
                />
              ) : null}
          ...
          <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-500">
              Al tocar un cargo o área vas a ver responsables, equipo, horas,
              función real y observaciones.
            </p>
          </div>
        )}
      </aside>
...
function InfoPill({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-2 rounded-2xl bg-slate-50 px-4 py-3 text-sm font-black text-slate-800">
      {icon}
      <span>{label}</span>
    </div>
  );
